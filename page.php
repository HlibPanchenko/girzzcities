...
// Проверяем только для CPT city-page
if ($post->post_type === 'city-page') {
    ...
    // Если это city-page и город не совпадает → 404
    if (!$city_matches) {
        global $wp_query;
        $wp_query->set_404();
        status_header(404);
    ...
    <div class="container">
        <?php if (!$title_disabled) { ?>
            <section class="wrapper">
                <?php if ($title_disabled === false || empty($title_disabled)) { ?>
